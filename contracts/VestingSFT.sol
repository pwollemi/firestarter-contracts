...
contract VestingSFT is Initializable, ERC721EnumerableUpgradeable {
    ...
    event VestingInfoUpdated(uint256 tokenId, uint256 totalAmount);

    /// @notice An event emitted when withdraw happens
    event Withdraw(uint256 tokenId, uint256 amountWithdrawn);
    ...
    function initialize(address _rewardToken, VestingParams memory _params) external initializer {
        __ERC721_init("Presale Vesting SFT", "Vesting");
    ...
    function updateRecipient(address recp, uint256 amount) external onlyOwnerOrWorker {
        ...
        vestingInfos[newTokenId].totalAmount = amount;

        emit VestingInfoUpdated(newTokenId, amount);
    }
    ...
    function withdraw(uint256 tokenId) external {
        require(msg.sender == ownerOf(tokenId), "withraw: must be the owner of this token");

        VestingInfo storage vestingInfo = vestingInfos[tokenId];
    ...
    function vested(uint256 tokenId) public view virtual returns (uint256 amount) {
        ...
        VestingInfo memory vestingInfo = vestingInfos[tokenId];
        ...
        uint256 withdrawnAmount = vestingInfos[tokenId].amountWithdrawn;
    ...
    function locked(uint256 tokenId) public view returns (uint256) {
        uint256 totalAmount = vestingInfos[tokenId].totalAmount;
